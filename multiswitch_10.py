...
class MultiSwitch(app_manager.RyuApp):
    OFP_VERSIONS = [ofproto_v1_0.OFP_VERSION]

    def __init__(self, *args, **kwargs):
        super(MultiSwitch, self).__init__(*args, **kwargs)
        self.mac_to_port = {}
        self.mac_to_sw = {}
        self.net = nx.DiGraph()
    ...
    def add_flow(self, datapath, priority, match, actions, buffer_id=None):
        ...
        if buffer_id:
            self.logger.info("=====> buff_id=%s", buffer_id)
    ...
    def get_topology_data(self, ev):
        ...
        self.logger.warn("add_nodes: %s" % (switches))
        self.net.add_nodes_from(switches)
        self.logger.warn("add_edges: %s" % (links))
        self.net.add_edges_from(links)
        for sw in backbone_ports:
            self.net.node[sw]['backbone_ports'] = backbone_ports[sw]
            self.logger.warn("backbone_ports[%s] = %s " % (sw, backbone_ports[sw]))
    ...
    def _packet_in_handler(self, ev):
        ...
        if dst_sw:
            ...
            for i in range(len(path)-1,-1,-1):
                ...
                self.logger.info("==> add_flow sw=%s buff_id=%s match_in_port=%s action_out_port=%s", sw, buff_id, match_in_port, action_out_port)
